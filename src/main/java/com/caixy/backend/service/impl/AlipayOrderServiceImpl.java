...
import com.caixy.backend.mapper.ProductOrderMapper;
import com.caixy.backend.model.dto.alipay.AliPayAsyncResponse;
...
import com.caixy.backend.model.enums.AlipayTradeStatusEnum;
...
import com.caixy.backend.service.*;
...
public class AlipayOrderServiceImpl extends ServiceImpl<ProductOrderMapper, ProductOrder> implements ProductOrderService
{
    ...
    public ProductOrderVo saveProductOrder(Long productId, UserVO loginUser)
    {
        ...
        BigDecimal scaledAmount =
                new BigDecimal(productInfo.getTotal()).divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
...
//        request.setReturnUrl(aliPayAccountConfig.getReturnUrl());
    ...
    public void processingTimedOutOrders(ProductOrder productOrder)
    {
        ...
        try
        {
            ...
            String tradeStatus =
                    AlipayTradeStatusEnum.findByName(alipayTradeQueryResponse.getTradeStatus()).getPaymentStatusEnum().getValue();
            ...
            {
                // 订单已支付更新商户端的订单状态
                boolean updateOrderStatus = this.updateOrderStatusByOrderNo(orderNo,
                        PaymentStatusEnum.SUCCESS.getValue());
                // 补发积分到用户钱包
                boolean addWalletBalance =
    ...
    public String doPaymentNotify(String notifyData, HttpServletRequest request)
    {
        ...
        AliPayAsyncResponse aliPayAsyncResponse =
                JSONUtil.toBean(JSONUtil.toJsonStr(params), AliPayAsyncResponse.class);
        String lockName = "notify:AlipayOrder:lock:" + aliPayAsyncResponse.getOutTradeNo();

        boolean distributedLock = redisOperatorService.tryGetDistributedLock(lockName, request.getRequestedSessionId(), 5L);
        if (!distributedLock)
        {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "【支付宝异步回调异常】:重复通知");
        }
        else {
            String result;
            try
    ...
    private String checkAlipayOrder(AliPayAsyncResponse response, Map<String, String> params) throws AlipayApiException
    {
        ...
        boolean verifyResult =
                AlipaySignature.rsaCheckV1(params, AliPayApiConfigKit.getAliPayApiConfig().getAliPayPublicKey(),
